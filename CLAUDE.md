...
- `config/settings.py` has `DEBUG = False` hardcoded and reads `SECRET_KEY` from the `QUANTBOT_SECRET_KEY` environment variable via `python-decouple` (fails loudly if unset) — this note previously said the opposite (`DEBUG = True`, hardcoded `SECRET_KEY`), which was stale and contradicted the actual, already-hardened settings; corrected by a forensic audit. Railway-aware SSL/CSRF hardening (`SECURE_SSL_REDIRECT`/`SESSION_COOKIE_SECURE`/`CSRF_COOKIE_SECURE`, gated on `DATABASE_URL` presence) is also already wired in.
